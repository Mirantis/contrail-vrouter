...
if re.search('^4\.', default_kernel_ver):
    ...
    kernel_build_dir = '/lib/modules/3.13.0-176-generic/build'
    if os.path.isdir(kernel_build_dir):
        default_kernel_ver = "3.13.0-176-generic"
        print "info: libdpdk will be built against kernel version %s" % default_kernel_ver
    else:
        print "*** Error: Cannot find kernel v3.13.0-176, build of vrouter will likely fail"
